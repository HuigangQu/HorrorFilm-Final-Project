// Update all visualizations function (modify existing function)
function updateVisualizations() {
    // Clear previous charts
    d3.select('#graph-container').selectAll('*').remove();
    d3.select('#comparison-container').selectAll('*').remove(); // Clear comparison charts
    ...
    // Create critic vs audience comparison charts
  ...
  function createComparisonChart(films, title, criticKey, audienceKey) {
    ...
    // Sort by difference magnitude (absolute value) in descending order
    validFilms.sort((a, b) => Math.abs(b.difference) - Math.abs(a.difference));
    
    // Take top 15 films with the biggest differences for better visualization
    const displayFilms = validFilms.slice(0, 15);
    
    // Create container for this chart
    const container = document.createElement('div');
    container.className = 'comparison-chart';
    document.getElementById('comparison-container').appendChild(container);
...
let comparisonState = {
  ...
  // Add this to your existing setupEventHandlers function
  function setupEventHandlers() {
    // Existing button handlers...
  
    // Add event handlers for comparison controls
    d3.select('#film-count').on('change', function() {
      comparisonState.filmCount = this.value;
      updateComparisonCharts();
    });
  
    d3.select('#sort-method').on('change', function() {
      comparisonState.sortMethod = this.value;
      updateComparisonCharts();
    });
  }
  
  // Add this new function to update just the comparison charts
  function updateComparisonCharts() {
    d3.select('#comparison-container').selectAll('*').remove();
    createCriticAudienceComparison();
  }
  
  // Update the createCriticAudienceComparison function
  function createCriticAudienceComparison() {
    // Create comparison for Rotten Tomatoes
    createComparisonChart(
      [...state.horrorFilms, ...state.nonHorrorFilms], 
  ...
  // Modify the createComparisonChart function to handle the new parameters
  function createComparisonChart(films, title, criticKey, audienceKey) {
    // Filter out films with missing data
    const validFilms = films.filter(d => 
      !isNaN(d[criticKey]) && !isNaN(d[audienceKey])
